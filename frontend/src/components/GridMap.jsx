...
export default function GridMap({
  ...
  function compareNodeColour(nodeId) {
    ...
    if (isPredicted && currentFrame >= (compareData?.end_idx ?? 0)) {
      if (isTP)  return '#06b6d4'; 
      if (isFP)  return '#9ca3af'; 
    }
    return null; 
  }
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      {/* ── Grid stats bar ──────────────────────────────────────────── */}
      <div className="shrink-0 flex items-center gap-6 px-4 py-2 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 text-xs transition-colors duration-300">
        <StatPill
          label="Generation"
        ...
        <StatPill
          ...
          colour={(gridStats.totalGen - gridStats.totalLoad) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}
        />
        ...
        {/* ── Compare-mode prediction summary ── */}
          ...
          return (
            <>
              ...
              <StatPill
                ...
                colour={actualCount > 0 ? 'text-cyan-600 dark:text-cyan-400' : 'text-gray-500 dark:text-gray-400'}
              />
            </>
          );
        })()}
        ...
        {tooltip && (
          <div className="absolute z-10 pointer-events-none bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-xs shadow-xl min-w-[170px] transition-colors"
            ...
            <div className="flex items-center justify-between mb-2 pb-1 border-b border-gray-200 dark:border-gray-700">
              <span className="font-bold text-gray-900 dark:text-white">Node {tooltip.node.id}</span>
              <span className={
                tooltip.node.is_failed ? 'text-red-600 dark:text-red-400' :
                tooltip.node.power_injection_mw > 0 ? 'text-green-600 dark:text-green-400' : 'text-blue-600 dark:text-blue-400'
              }>
            ...
            {compareMode && compareData && (
              <div className="mb-2 pb-1 border-b border-gray-200 dark:border-gray-700 space-y-0.5">
                {predictedNodeIds.has(tooltip.node.id) && (
                  <p className="text-purple-600 dark:text-purple-400 font-semibold">⬡ Predicted failure</p>
                )}
                {revealedGtNodeIds.has(tooltip.node.id) && (
                  <p className="text-red-600 dark:text-red-400 font-semibold">⚡ Actual failure</p>
                )}
                {compareData.metrics.true_positives.includes(tooltip.node.id) && (
                  <p className="text-cyan-600 dark:text-cyan-400">✓ Correct prediction</p>
                )}
                {compareData.metrics.false_positives.includes(tooltip.node.id) && (
                  <p className="text-gray-500 dark:text-gray-400">False alarm</p>
                )}
                {compareData.metrics.false_negatives.includes(tooltip.node.id) && (
                  <p className="text-orange-600 dark:text-orange-400">⚠ Missed by model</p>
                )}
              </div>
            )}
            <div className="mb-2 space-y-0.5">
              <TRow label="P" value={`${tooltip.node.power_injection_mw.toFixed(1)} MW`}
                colour={tooltip.node.power_injection_mw >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'} />
              <TRow label="Q" value={`${tooltip.node.reactive_injection_mvar.toFixed(1)} MVAr`} colour="text-blue-600 dark:text-blue-400" />
            </div>
        ...
        <div className="shrink-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 px-4 py-3 space-y-2 transition-colors duration-300">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => stepFrame(-1)}
              className="w-7 h-7 flex items-center justify-center rounded bg-gray-200 hover:bg-gray-300 text-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-300 text-sm transition-colors">
              ‹
            </button>
            <button onClick={togglePlay}
              className="w-8 h-8 flex items-center justify-center rounded bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600 text-white font-bold transition-colors">
              {playing ? '⏸' : '▶'}
            </button>
            <button onClick={() => stepFrame(1)}
              className="w-7 h-7 flex items-center justify-center rounded bg-gray-200 hover:bg-gray-300 text-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-300 text-sm transition-colors">
            ...
            {compareData.cascade_detected && compareData.cascade_start_time >= 0 && (() => {
              ...
              return (
                <span className="px-2 py-0.5 rounded bg-cyan-100 text-cyan-700 dark:bg-cyan-900 dark:text-cyan-300 text-xs font-semibold">
...
function TRow({ label, value, colour = 'text-gray-700 dark:text-gray-300' }) {
  return (
...
function tooltipVoltageColour(pu) {
  if (pu < 0.90 || pu > 1.10) return 'text-red-600 dark:text-red-400';
  if (pu < 0.95 || pu > 1.05) return 'text-yellow-600 dark:text-yellow-400';
  return 'text-green-600 dark:text-green-400';
}

function tooltipFreqColour(hz) {
  if (hz < 59.0 || hz > 61.0) return 'text-red-600 dark:text-red-400';
  if (hz < 59.5 || hz > 60.5) return 'text-yellow-600 dark:text-yellow-400';
  return 'text-green-600 dark:text-green-400';
}

function tooltipTempColour(c) {
  if (c > 85) return 'text-red-600 dark:text-red-400';
  if (c > 70) return 'text-yellow-600 dark:text-yellow-400';
  return 'text-green-600 dark:text-green-400';
}

function tooltipCondColour(v) {
  if (v < 0.40) return 'text-red-600 dark:text-red-400';
  if (v < 0.70) return 'text-yellow-600 dark:text-yellow-400';
  return 'text-green-600 dark:text-green-400';
}